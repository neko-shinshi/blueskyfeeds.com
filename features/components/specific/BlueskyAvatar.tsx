...
export default function BlueskyAvatar ({type, avatar, uri}:{type:"user"|"feed", avatar:string, uri:string}) {
    return <>
        {
            type === "feed" && <div className="w-14 h-14 aspect-square hover:blur-xs relative">
        ...
        {
            type === "user" && <div className="w-12 h-12 aspect-square relative">
                <a className="hover:blur-xs" href={`https://bsky.app/profile/${uri}`}>
                    <Image unoptimized fill
                        src={avatar}
                        className="inset-0 rounded-full text-transparent"
                        alt='User Image'
                    />
                </a>
            </div>
